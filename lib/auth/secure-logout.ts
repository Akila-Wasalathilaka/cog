...
export class SecureLogout {
  ...
  static setupPageProtection(): void {
    ...
    window.addEventListener('popstate', function(_event) { // eslint-disable-line @typescript-eslint/no-unused-vars
      const token = localStorage.getItem('access_token') || sessionStorage.getItem('access_token');
 ...
 */
export function validateTokenAndRole(requiredRole?: 'ADMIN' | 'CANDIDATE'): { isValid: boolean; payload?: any; error?: string } {
  try {
    ...
    if (requiredRole && payload.role?.toUpperCase() !== requiredRole) {
      return { isValid: false, error: `Access denied. Required role: ${requiredRole}` };
    }
