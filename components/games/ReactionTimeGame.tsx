...
// Fallback stimuli if none provided
...
export default function ReactionTimeGame({ config, onComplete, onProgress }: ReactionTimeProps) {
  ...
  const generateTrials = useCallback(() => {
    ...
    const stimuli = config.stimuli && config.stimuli.length > 0 ? config.stimuli : fallbackStimuli;
  ...
  const presentNextStimulus = useCallback((trialList: ReactionTimeTrial[], trialIndex: number) => {
    if (trialIndex >= trialList.length) {
      ...
      const validTrials = trialList.filter(t => t.responseTime && t.responseTime > 0);
      const averageResponseTime = validTrials.length > 0
        ? validTrials.reduce((sum, t) => sum + (t.responseTime || 0), 0) / validTrials.length
        : 0;

      const accuracy = trialList.length > 0 
        ? Math.max(0, Math.min(100, (validTrials.length / trialList.length) * 100)) 
        : 0;

      // Calculate score considering both accuracy and response time
      let timeScore = 0;
      if (averageResponseTime > 0) {
        // For reaction time, faster is better (ideal is 200-400ms)
        if (averageResponseTime < 200) {
          timeScore = 80; // Very fast but might be anticipating
        } else if (averageResponseTime < 300) {
          timeScore = 100; // Ideal reaction time
        } else if (averageResponseTime < 400) {
          timeScore = 95; // Good reaction time
        } else if (averageResponseTime < 500) {
          timeScore = 85; // Acceptable reaction time
        } else if (averageResponseTime < 600) {
          timeScore = 70; // Slow reaction time
        } else {
          timeScore = 50; // Very slow
        }
      }
      ...
      const results = {
        ...
        completedTrials: validTrials.length,
        correctTrials: validTrials.length, // All valid responses are correct
        accuracy: accuracy,
